...
# ╔═╡ 45c5c860-e666-4555-b0a3-16151b15a757
md"""
## Simplifying and Solving this System
Now, this system could be solved directly, as a DAE.
We would use [one of the DAE solvers from DifferentialEquations.jl](https://docs.sciml.ai/DiffEqDocs/stable/solvers/dae_solve/).
But don't forget!
The power of MTK goes well beyond "just" making building models more fun.
Let's take a second first to *symbolically* simplify the system before doing the solve.

Going on a tangent, we could solve the RC model using an ODE solver that can handle mass matrices.
We could plug the initial state and the mass matrix to a suitable ODE solver (e.g. the generalized-α or the Houbolt method) and solve the system numerically.
However, running the infamous (I hope, by now) function `structural_simplify` first, eliminates many simply unnecessary variables to build the leanest numerical representation of the system.
Check this out:
"""
...
md"""
Compare that to the equations and states of `rc_model`.
Yeah.
...
md"""
Can we do better?
The answer to this question is almost always yes because `structural_simplify` is the best.
Because we've run `structural_simplify`, MTK can numerically solve all the unreduced algebraic equations numerically using the `ODAEProblem` (note the letter `A`):
"""
...
What if we wanted to plot the timeseries of a different variable?
Not to worry, transformations like `structural_simplify` simply change state variables into `observed` variables.
Let's take a look:
"""
...
git-tree-sha1 = "39a56ae18a139f93a544c2cefef048b3932f6e19"
