...
# ╔═╡ 081d64cd-79f8-4d0c-8ea3-e681c4470a7c
begin
	using PlutoUI
	TableOfContents(title = "Acausal modeling: The sequel")
end

# ╔═╡ a0fe4dcf-b575-40b3-b0d5-ecef17d47246
begin
...
# ╔═╡ 58c7c4fa-0a66-4941-8bd3-278fda3988b6
md"""
## Introduction
This is a short notebook to briefly touch on a couple points made in the [acausal notebook](https://xlxs4.github.io/modeling-toolkit-exploration/acausal.html).
I'll probably make separate notebooks to go over each point separately and more in-depth.
I'll probably remember to link them back to here when the time comes.
Or I might not.
Let's dive in.
"""

# ╔═╡ 7dacfcdc-56da-4b63-9237-68229e5d7daf
md"""
## Component Library
In the ["Building the Component Library"](https://xlxs4.github.io/modeling-toolkit-exploration/acausal.html#f86c4eac-5344-4b3c-9a8e-c49c1d54cba1) section of the acausal notebook, I close by saying:

> Let's reflect upon what we did. We've created a bunch of layered abstractions and ultimatelly defined each and every one of the component we need. Because it's all just Julia code, nothing stops us from putting them in a file and exporting the definitions. That way, the next time we'll need to model something that also consists of a Resistor, we'll be able to reuse what we already have. That's so cool! It means every time we model something, we involuntarily expand our own personal library of reusable components to build models with! And, of course, nothing stops you from sharing these. The world is your oyster.

Let's address some of these points.
"""

# ╔═╡ 91b9c9f5-0994-4099-9306-9659b4754075
md"""
> Because it's all just Julia code, nothing stops us from putting them in a file and exporting the definitions.

This also allows us to further leverage abstraction strategies.
Being able to split the component definitions in various files, or modules, or packages and use them as we please is very powerful.
It allows for maintainable, modular and easy-to-share models.
What if you do your thesis on [Swarmalators](https://www.complexity-explorables.org/explorables/swarmalators/) or [Shearlets](https://www.wikiwand.com/en/Shearlet) and you find yourself defining some components?
Maybe someone else can use them too.
And what if you could write code to implement an interface that takes a component and converts it so that it can be imported and used in other modeling frameworks?
What if you could do the reverse, too?
And what if you could write a library, just the way [Catalyst](https://github.com/SciML/Catalyst.jl) is for chemical reaction networks, and you could define your own set of components to be used throughout the packages of your ecosystem?
Pretty cool stuff.
"""

# ╔═╡ ed95d44e-32d2-4956-aa5d-74589403713b
md"""
...
I think there's a key observation to be made here.
Hop on for a quick journey.
So there's this concept some refer to as ["Second brain"](https://fortelabs.com/blog/basboverview/).
Essentially, it's a DIY methodology to end up with a system that allows you to hoard knowledge which accumulates through time, while on the same hand making sure that it's easily searchable and that it's intuitive enough to recover even when you know it's needed.
At its core, since it pertains to knowledge, it revolves around note collecting and notetaking strategies.
Speaking of notetaking strategies, there's this thing called [sketchnoting](https://www.verbaltovisual.com/what-is-sketchnoting/): notetaking using mostly visual elements, like quick sketches or drawings.
I touch upon this in [this article](https://xlxs4.github.io/notes/second-brain/#sketchnoting).
The idea is that I can use digital drawings or images from the web to create my notes.
From [another section](https://xlxs4.github.io/notes/second-brain/#second_brain) in the article:

> I want to transition to sketchnoting, hopefully accumulating a vocabulary of atomic, small drawings that I can use in a modular fashion.

So there's some parallels to be drawn with the seemingly unrelated topics that are sketchnoting and MTK components.

First, you can leverage abstraction to have atomic, self-contained, modular and extendable functional units, yada yada yada.
Second, you can *compose* these to be able to build more complicated entities with less work.
Third, the "less work" part cannot be understated. From the article:
> Eventually as your system matures, everything should ideally be almost a remix; minimize new research. Mix and match already conquered knowledge, apply already-made observations instead. Build different things from the same blocks.

Imagine being able to reuse a variety of model components, subsystems, instead than always starting from scratch.

Lastly, I can't stress this enough. No matter what you do, no matter what you work on, every single thing that you do gains a lot more value because it necessarily contributes to a constantly-growing mass of information that you can draw from at your whim. Every single thing that you do, every few lines of code that you write. They're yours to keep and use, forever.
From the acausal notebook:

> [...] every time we model something, we involuntarily expand our own personal library of reusable components to build models with!

Are we done?
Well, sort of.
We could go on and on forever, but let's milk this very prolific cow a little bit longer.
Let's go over to the newly emerging field of Synthetic Biology, or SynBio for a moment.

> Synthetic biology is a growing discipline that has two subfields. One uses unnatural molecules to reproduce emergent behaviors from natural biology, with the goal of creating artificial life. **The other seeks interchangeable parts from natural biology to assemble into systems that act unnaturally.** [^1] (emphasis mine)
...
[^2]: Lee, M. E., DeLoache, W. C., Cervantes, B., & Dueber, J. E. (2015). A highly characterized yeast toolkit for modular, multipart assembly. *ACS synthetic biology, 4*(9), 975-986.
[^3]: Abelson, H., & Sussman, G. J. (1996). *Structure and interpretation of computer programs* (p. 688). The MIT Press.
[^4]: Hanson, C., & Sussman, G. J. (2021). *Software Design for Flexibility: How to Avoid Programming Yourself into a Corner*. MIT Press.
[^5]: McAllister, W. H., Rozas, G. J., Sussman, G. J., & Wisdoms, J. (1994). The Supercomputer Toolkit: A General Framework for Special-Purpose Computing.
[^6]: Sussman, G. J., & Wisdom, J. (1992). Chaotic evolution of the solar system. *Science, 257*(5066), 56-62.
"""

# ╔═╡ 6324888d-2288-48b1-be24-0692dceb88cb
md"""
## Standard Library
I'll keep this short.
MTK is amazing.
Therefore MTK also comes with the amazing [Standard Library](https://github.com/SciML/ModelingToolkitStandardLibrary.jl) that, admittedly, *could* use a better name.
In the acausal notebook, we defined components from scratch, such as the `Pin` connector, the `OnePort` abstraction and the `Resistor` to [give shape to our RC circuit](https://xlxs4.github.io/modeling-toolkit-exploration/acausal.html).
We can use the standard library, and specifically its [Electrical Components](https://docs.sciml.ai/ModelingToolkitStandardLibrary/stable/API/electrical/), to build our RC circuit.
In fact, every single component that we built there is available through the standard library, including the `Pin` `@connector`.
So let's instead do something slightly more cooler and build a Chua circuit.
"""

# ╔═╡ e9aa278f-d67a-4622-8922-93dd25f51ddc
md"""
### Chua Circuit
...
The ease of construction of the circuit has made it a ubiquitous real-world example of a chaotic system, leading some to declare it "a paradigm for chaos" [^7].
...
[^7]: Madan, R. N. (Ed.). (1993). *Chua's circuit: a paradigm for chaos* (Vol. 1). World Scientific.
"""

# ╔═╡ a19e0dd9-825a-4832-bba6-206308025a3b
md"""
Let's first get our imports out of the way:
"""

# ╔═╡ ed16dad3-574b-455f-8467-e925d7cb2dfb
md"""
Now let's define our components, or rather, component, since everything other than the nonlinear element is already taken care of by the standard library:
"""

# ╔═╡ 4172a4bc-eb86-4d97-88d4-025b9384a9f3
@parameters t;

# ╔═╡ 7e163e36-4f39-4ce0-9a6c-f87423e604cb
...
# ╔═╡ b95eb005-3f40-4134-af9d-13743469775b
md"""
Now let's actually build the components that we need:
"""

# ╔═╡ f1cfc0bd-6b94-41ca-8b4a-698bc51fa093
begin
...
# ╔═╡ edb1c766-750f-4759-abf4-d8debbb2ba3e
md"""
Connect everything together, just like an Electrical Engineer would do IRL:
"""
...
ModelingToolkitStandardLibrary = "~1.9.0"
...
git-tree-sha1 = "004138f0ec3bedc35c0321a9cf1836415641d1f8"
...
git-tree-sha1 = "cbb9983128709b971d624fbcd27c9a7e61bb41f5"
